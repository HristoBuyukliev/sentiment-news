...
weight_path = "attention_weights.hdf5"
checkpoint = ModelCheckpoint(weight_path, monitor='val_loss',
                             verbose=1, save_best_only=True, mode='min')
early_stopping = EarlyStopping(monitor="val_loss", mode="min", patience=5)
callbacks = [checkpoint, early_stopping]

# model.fit(X_train, train.labels, batch_size=batch_size, epochs=epochs,
...
predictions_df = pandas.DataFrame({'attention_preds': model.predict(X_test)})
predictions_df.to_csv('attention_preds.csv')
